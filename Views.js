const ListsView = () => {
  ...
  const handleAddBook = useCallback((fid) => {
          ...
          <>
            ...
            {books.length === 0 && folders.length === 0 && !isAddingRoot ? (
              ...
              <div className="space-y-1 min-h-[60px] rounded-xl transition-colors" data-folder-target="root">
                  <BookList ids={rootBooks.map(b => b.id)} books={books} folderKey="root" onOpen={handleOpenBook} showIndicator={true} />
                  {rootFolders.map(folder => <FolderNode key={folder.id} folder={folder} allFolders={folders} allBooks={books} onAddBook={handleAddBook} onOpenBook={handleOpenBook} />)}
              </div>
            )}
          </>
        )}
     ...
     <div className="flex-1 overflow-y-auto p-4 pb-24" data-dnd-scroll data-folder-target="root">
        {isSearching ? (
          ...
          <div className="space-y-1" data-folder-target="root">
             <BookList ids={rootBooks.map(b => b.id)} books={libraryBooks} folderKey="root" onOpen={handleOpenBook} isLibraryView={true} />
            {rootFolders.map(folder => <FolderNode key={folder.id} folder={folder} allFolders={visibleFolders} allBooks={libraryBooks} onOpenBook={handleOpenBook} isLibraryView={true} />)}
          </div>
        )}
      </div>
      <BookDetailModal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} bookId={activeBookId} />
