...
const BookCard = ({ book, onOpen, showIndicator = false, folderPath = null, onNavigate = null, containerFolderId = null }) => {
  ...
  const handleTouchMove = (e) => {
    e.preventDefault(); // Stop native scroll while dragging from handle
  };
  ...
  return (
    <div
      ...
      className={`group flex items-center p-2 pr-3 bg-white border border-zinc-100 rounded-xl shadow-sm hover:border-zinc-300 ml-2 sm:ml-4 transition-colors my-1.5 select-none ${isBeingDragged ? 'opacity-0' : ''}`}
    >
      {/* DRAG HANDLE */}
      {containerFolderId && (
        <div
          className="p-2 mr-1 text-zinc-300 hover:text-zinc-500 cursor-grab active:cursor-grabbing shrink-0"
      ...
      >
        ...
        {showIndicator && book.inLibrary && (
          <span className="ml-2 w-2 h-2 rounded-full bg-zinc-900 shrink-0" title="Kütüphanemde"></span>
        )}
      </div>
    </div>
  );
