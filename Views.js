...
const BookCard = ({ book, onOpen, showIndicator = false, folderPath = null, onNavigate = null, containerFolderId = null }) => {
  ...
  // ── Scroll kilitleme / açma ──
  const lockScroll = () => {
    const sc = getScrollContainer();
    sc._savedOverflow = sc.style.overflowY;
    sc._savedTouchAction = sc.style.touchAction;
    sc._savedOverscroll = sc.style.overscrollBehavior;
  ...
  const unlockScroll = () => {
    const sc = getScrollContainer();
    if (sc._savedOverflow !== undefined) {
      sc.style.overflowY = sc._savedOverflow;
      sc.style.touchAction = sc._savedTouchAction || '';
      sc.style.overscrollBehavior = sc._savedOverscroll || '';
      delete sc._savedOverflow;
      delete sc._savedTouchAction;
      delete sc._savedOverscroll;
    }
  ...
  // ── Auto-scroll: kenar yakınında otomatik kaydırma ──
  const EDGE_ZONE = 60;  // kenardan kaç px uzaklıkta aktif olsun
  const MAX_SCROLL_SPEED = 8; // piksel/frame
  ...
  const startAutoScroll = (clientY) => {
    ...
    if (topDist < EDGE_ZONE && sc.scrollTop > 0) {
      speed = -MAX_SCROLL_SPEED * (1 - topDist / EDGE_ZONE); // yukarı
    } else if (bottomDist < EDGE_ZONE && sc.scrollTop < sc.scrollHeight - sc.clientHeight) {
      speed = MAX_SCROLL_SPEED * (1 - bottomDist / EDGE_ZONE); // aşağı
    }

    if (Math.abs(speed) > 0.5) {
      // Scroll kilidi geçici olarak aç, programatik scroll yap, tekrar kapat
  ...
  // touchmove engelleme artık global _isDragActive flag'i ile yapılıyor (dosya başı)
  ...
  const handlePointerMove = (e) => {
    ...
    // ── Finger jitter algılama ──
    // Long-press beklerken parmak hareket etti mi?
    ...
    // Karar mantığı:
    // 1) 15px altı → jitter, hiçbir şey yapma (timer devam etsin)
    // 2) 15-30px arası VE hızlı (>0.15 px/ms) → kasıtlı scroll, timer iptal
    // 3) 30px üstü → kesinlikle scroll, timer iptal
    if (dist > 30 || (dist > 15 && velocity > 0.15)) {
  ...
  // ── Temizlik ──
  const stopTracking = () => {
    _isDragActive = false; // Global scroll blocker'ı kapat
  ...
  const handlePointerDown = (e) => {
    ...
    // CRITICAL: touch-action hemen kilitle, tarayıcı scroll kararı vermeden önce
    if (cardRef.current) {
    ...
    pressTimer.current = setTimeout(() => {
      if (!movedRef.current && cardRef.current) {
        ...
        _isDragActive = true; // Global scroll blocker'ı aç
