      ...
      <div className="p-4 pt-6 pb-3 sticky top-0 bg-white/90 backdrop-blur-md z-10 border-b border-zinc-100 shadow-sm min-h-[70px] flex items-center">
        {isSearching ? (
          ...
          <div className="flex w-full justify-between items-center">
            <div className="flex items-center gap-1.5 text-[15px] font-semibold text-zinc-600 overflow-x-auto whitespace-nowrap hide-scrollbar flex-1 mr-2">
              <button onClick={() => setActiveFolderId(null)} className={`hover:text-zinc-900 transition-colors flex items-center gap-1 ${!activeFolderId ? 'text-zinc-900 text-2xl font-bold tracking-tight' : ''}`} data-folder-target="root">
                {!activeFolderId && <Library size={24} className="mr-1" />}
                Listelerim
              </button>
              {breadcrumbs.map((bc) => (
                <React.Fragment key={bc.id}>
                  <ChevronRight size={16} className="opacity-40 shrink-0" />
                  <button onClick={() => setActiveFolderId(bc.id)} className={`hover:text-zinc-900 transition-colors ${activeFolderId === bc.id ? 'text-zinc-900 text-xl font-bold tracking-tight' : ''}`} data-folder-target={bc.id}>
                    {bc.name}
                  </button>
                </React.Fragment>
              ))}
