        ...
        {isSearching ? (
          ...
          <div className="flex w-full justify-between items-start">
            <div className="flex flex-col flex-1 mr-2">
              <button onClick={() => setActiveFolderId(null)} className={`text-left flex items-center gap-1.5 transition-colors w-max px-1.5 py-0.5 rounded-lg ${!activeFolderId ? 'text-zinc-900 font-bold text-xl tracking-tight bg-zinc-50' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 font-semibold text-[15px]'}`} data-folder-target="root">
                {!activeFolderId && <Library size={20} className="mr-1" />}
                Listelerim
              </button>
              {breadcrumbs.map((bc, idx) => (
                <div key={bc.id} className="flex items-center mt-1" style={{ marginLeft: `${(idx + 1) * 16}px` }}>
                  ...
                  <button onClick={() => setActiveFolderId(bc.id)} className={`text-left transition-colors w-max px-1.5 py-0.5 rounded-lg ${activeFolderId === bc.id ? 'text-zinc-900 font-bold text-xl tracking-tight bg-zinc-50' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 font-semibold text-[15px]'}`} data-folder-target={bc.id}>
                    {bc.name}
                  </button>
                </div>
              ))}
