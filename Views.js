...
const ListsView = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-white relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24" onClick={() => setActiveFolder(null)}>
        <DragDropProvider onDrop={(bId, fId) => moveBookToPosition(bId, fId)}>
          <RootDropZone>
            {rootBooks.map(b => <BookCard key={b.id} book={b} onOpen={() => {}} draggable={true} />)}
            {rootFolders.map(f => <FolderNode key={f.id} folder={f} allFolders={folders} allBooks={books} onOpenBook={() => {}} draggable={true} activeFolder={activeFolder} setActiveFolder={setActiveFolder} onAddBook={setActiveFolder} />)}
          </RootDropZone>
        </DragDropProvider>
      </div>

      <button onClick={() => setSearchModalOpen(true)} className="fixed bottom-6 right-6 w-14 h-14 bg-zinc-900 text-white rounded-full shadow-xl flex items-center justify-center hover:scale-105 transition-all z-50">
        <Plus size={28} />
      </button>
...
/* --- DİĞER BİLEŞENLER (LibraryView, StatsView vb.) BURAYA AYNEN DEVAM EDECEK --- */
