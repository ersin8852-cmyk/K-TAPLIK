const { useLayoutEffect, useRef, memo } = React;

const BookList = memo(({ ids, books, folderKey, onOpen, showIndicator = false, isLibraryView = false }) => {
  ...
  if (draggedId && overTarget) {
    if (overTarget.type === 'book' && overTarget.folderId === folderKey) {
      const idx = visibleIds.indexOf(overTarget.id);
      if (idx !== -1) previewIndex = overTarget.placement === 'after' ? idx + 1 : idx;
    } else if (overTarget.type === 'folder' && overTarget.id === folderKey) {
