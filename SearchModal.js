const SearchAddModal = ({ isOpen, onClose, folderId }) => {
  ...
  useEffect(() => {
    if (showCamera && cameras.length === 0) {
      window.Html5Qrcode.getCameras().then(devices => {
        if (devices && devices.length > 0) {
          setCameras(devices);
          const envCam = devices.find(c => c.label.toLowerCase().includes('back') || c.label.toLowerCase().includes('arka'));
          setSelectedCameraId(envCam ? envCam.id : devices[0].id);
        }
      }).catch(err => {
        showToast('Kameralar alınamadı.', 'error');
      });
    }
  }, [showCamera, cameras.length]);

  useEffect(() => {
    if (showCamera && selectedCameraId) {
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-lg overflow-hidden flex flex-col h-[85vh] sm:h-[80vh] shadow-2xl animate-in slide-in-from-bottom-10">
        ...
        {showCamera && (
          <div className="px-4 pb-2 shrink-0 flex flex-col gap-2">
            {cameras.length > 1 && (
              <select 
                className="w-full p-2 border border-zinc-200 rounded-xl bg-zinc-50 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-800 text-zinc-700"
